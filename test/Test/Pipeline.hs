...
spec = describe "Full Pipeline" $ do
  ...
  it "should transform notebook code to valid C++ through full pipeline" $ do
    ...
    (siftedDoc, _ :: DeclarationSifter) <- project (DeclarationSifterParams "minimal-parser" False "") inputDoc
  ...
  it "should work with combined DeclarationSifter that wraps executables" $ do
    ...
    (siftedWrappedDoc, sifter :: DeclarationSifter) <- project (DeclarationSifterParams "minimal-parser" True "__notebook_exec") inputDoc
    ...
    -- Then add main wrapper
    (finalDoc, _ :: HeadTailTransformer) <- project ([], ["", "int main() {", "  __notebook_exec();", "  return 0;", "}"]) siftedWrappedDoc
    let finalText = T.intercalate "\n" $ docToList finalDoc

    -- Verify final output has everything
    T.isInfixOf "int main() {" finalText `shouldBe` True
    T.isInfixOf "__notebook_exec();" finalText `shouldBe` True
