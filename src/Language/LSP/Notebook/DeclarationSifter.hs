...
instance Transformer DeclarationSifter where
  ...
  getParams (DeclarationSifter _) = DeclarationSifterParams "minimal-parser"
  ...
  project params doc = do
    ...
    case result of
      Left _err -> return (doc, DeclarationSifter V.empty) -- fallback to no transformation
      Right declarations -> do
        let (siftedLines, remainingLines, siftedIndices) = siftDeclarations originalLines declarations
        return (listToDoc (siftedLines ++ remainingLines), DeclarationSifter (V.fromList siftedIndices))

  transformPosition :: Params DeclarationSifter -> DeclarationSifter -> Position -> Maybe Position
  transformPosition _ (DeclarationSifter indices) = transformUsingIndices indices

  untransformPosition :: Params DeclarationSifter -> DeclarationSifter -> Position -> Maybe Position  
  untransformPosition _ (DeclarationSifter indices) = Just . untransformUsingIndices indices
