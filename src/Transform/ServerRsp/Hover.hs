...
fixupDocumentReferences :: forall n. MonadLogger n => Regex -> CppNotebookTransformer -> Rope -> HoverContents -> n HoverContents
...
fixupDocumentReferences' :: forall n. MonadLogger n => Regex -> CppNotebookTransformer -> Text -> n Text
fixupDocumentReferences' docRegex transformer t =
  traverseOf ((regexing docRegex) . groups) (transformGroup transformer) t

  where
    transformGroup :: CppNotebookTransformer -> [Text] -> n [Text]
